...
## D-075 — Capacity per service (RATIFIED with corrections, 2026-08-17)
Direction ratified by Codex; build-ready contract in `docs/specs/capacity-model.md`. Every service has a capacity model. The two archetypes are PRESETS/config (NOT a persisted enum) over the stable `capacity_model = bounded | unlimited` + versioned `capacity_config`: occupancy-numeric (boarding, daycare) persist a REQUIRED positive `capacity_config.service_limit` (no separate default_capacity; supersedes any "pool-only" option); staff/window services (walking, drop-in, in-home sitting) derive capacity from config + staff + windows (zones constrain, don't multiply; staff counted once across overlaps; sitting = exclusive-engagement). Shared physical pool (`capacity_group_id`, SEPARATE from scheduling `conflict_group_id`, tenant-composite FK) required ONLY when 2+ co-located services share a finite resource. Occupancy counting (Danny's ruling, overriding Codex's daytime-presence recommendation): a dog counts on arrival + every night, NOT on the departure date — `[arrival_service_date, departure_service_date)` half-open in business tz; same date-based projection for the service cap AND the pool. Boarding occupies `[arrival, departure)`; daycare occupies exactly its single `service_date` bucket (exclusive upper bound `service_date + 1 day`) so a same-day daycare booking still occupies one date, never zero. The shared pool is a date-bucket scheduling limit with a departure-day tolerance (a departing animal is uncounted for its whole departure date; an arriving animal counts its whole arrival date), NOT a guaranteed instantaneous physical ceiling; atomic service-and-pool enforcement remains required. Lands with CFG-1 (tenant + RLS), not the waitlist migration.

## D-076 — Availability calendar: per-day management (RATIFIED with corrections, 2026-08-17)
Right-of-calendar daily form per date: "Services Available" vs "Block All" (replaces Open / Limit Services / Block All). Per-day overrides use TARGET-SPECIFIC tables (single overloaded row rejected), keyed `business_service_id` + `service_date`: `business_calendar_days` (all_services_blocked, holiday_pricing), `business_service_day_overrides` (is_available, service_limit_override), `capacity_group_day_overrides` (pool_limit_override), `service_window_day_overrides` (+ assignments). "Block All" = one day-level flag covering later-added services. Closure = is_available=false (not capacity 0); positive capacities. Reset = delete override row + audit event; lowering below confirmed occupancy flags over-capacity (never cancels). Precedence: Block All → service/window disabled → per-day limit → base default. `holiday_pricing` only selects the rate set (pricing stays in `packages/pricing`). Canonical: `docs/specs/capacity-model.md`.


## D-075 / D-076 — Correction (RATIFIED, Codex, 2026-08-18)
Walking/drop-in capacity is SUMMED: window cap = Σ assigned walkers' effective caps (per-walker default "Dogs Per Walk" + optional per-window override); zone cap = Σ caps of covering walkers. This SUPERSEDES the uniform "dogs-per-walker × walkers" formula in the 2026-08-17 ratification. A walk window is a first-class named sub-service (Solo = business-scoped; Duo+ = day/time/staff). Service zones attach at the whole-service AND sub-service/window/walker levels, selecting from a REUSABLE per-tenant zone pool (not a private set per service). Codex accepted the deviation (no engine blocker) and the canonical spec + schema deltas were rewritten to one contract; concrete `capacity_config` + zone-keying (service / window / staff) land at CFG-1 build. Canonical: `docs/specs/capacity-model.md`.
