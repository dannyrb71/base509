...
- **LIVE 2026-08-16 (evening):** base509.com + petappro.com (and both www) are live on Vercel. Danny added the 4 Cloudflare CNAMEs (→ 47c4a688e7208c90.vercel-dns-017.com, DNS-only); all 4 verify "configured-correctly", SSL issued, HTTPS 200 on all hosts. app.petappro.com NOT pointed (D-061, portal dark). Google MX / Resend records untouched. Cleanup: Danny can revoke the Cloudflare API token (never got write perms; unused) and the Supabase access token when convenient.
- **Launch-night fixes 2026-08-16:** `cd543fd` middleware canonicalization (branded domains 308 brand-prefixed paths → clean URLs; cross-brand hops to the other domain; fixed all nav on petappro.com/base509.com) · `e570444` footer Base509 logo links to base509.com · `c538a99` feature spotlight carousel slides L/R instead of hard-swapping (reduced-motion safe). All verified live. Follow-ups logged: clean-href refactor to remove 308 hops; header "Sign In → Notify Me" form fakes success without saving (wire to /api/waitlist).
- **Branded email live 2026-08-16 (late):** `a653e97` HTML waitlist confirmation (template inlined in src/lib/waitlist-email.ts, text fallback kept; paw/instagram/facebook PNGs on petappro.com all 200). ⚠️ Found+fixed in the process: `a29e5d4` — middleware was brand-prefixing /api/*, so the live waitlist form 404'd on the real domains since DNS cutover (previews unaffected). Verified end-to-end on petappro.com: signup 200 → branded email rendered in Gmail → test row deleted.
