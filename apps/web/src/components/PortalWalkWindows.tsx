                ...
                <article className={!window.active ? 'is-disabled' : undefined} key={window.id}>
                  ...
                  {expanded && <><div className="portal-walk-window-grid">
                    ...
                    {hasTeamAssignments && <fieldset className="portal-walk-window-walkers"><legend className="type-label">Walkers Assigned</legend><div className="portal-walker-assignment-list">{WALKERS.map((walker) => {
                      ...
                      return <div className={assigned ? 'portal-walker-assignment is-active' : 'portal-walker-assignment'} key={walker}><label className="portal-checkbox type-body"><input type="checkbox" checked={assigned} onChange={() => toggleWindowListValue(window.id, 'walkers', walker)} /><span aria-hidden="true" />{walker}</label>{assigned && <div><div className="portal-walker-dogs"><small className="type-caption">Dogs This Window</small><div className="portal-stepper-input"><button className="portal-step-button" type="button" aria-label={`Fewer dogs for ${walker} in ${window.name}`} onClick={() => updateWindow(window.id, { walkerDogs: { ...window.walkerDogs, [walker]: String(Math.max(1, capacityFor(window, walker) - 1)) } })}><Image src="/brands/petappro.com/icon-minus.svg" alt="" width={24} height={24} /></button><input type="number" min="1" inputMode="numeric" aria-label={`Dogs per walk for ${walker} in ${window.name}`} value={window.walkerDogs[walker] ?? walkerCapacities[walker] ?? ''} onChange={(event) => updateWindow(window.id, { walkerDogs: { ...window.walkerDogs, [walker]: event.target.value } })} /><button className="portal-step-button" type="button" aria-label={`More dogs for ${walker} in ${window.name}`} onClick={() => updateWindow(window.id, { walkerDogs: { ...window.walkerDogs, [walker]: String(capacityFor(window, walker) + 1) } })}><Image src="/brands/petappro.com/icon-plus.svg" alt="" width={24} height={24} /></button></div></div><small className="type-caption">Coverage Zones</small><div className="portal-option-list">{window.zones.map((zone) => <button className={`portal-chip portal-chip--compact${(window.walkerZones[walker] ?? []).includes(zone) ? ' is-active' : ''}`} type="button" aria-pressed={(window.walkerZones[walker] ?? []).includes(zone)} key={zone} onClick={() => toggleWalkerZone(window.id, walker, zone)}>{zone}</button>)}</div></div>}</div>;
