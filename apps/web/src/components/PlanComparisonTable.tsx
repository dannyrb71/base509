...
export function PlanComparisonTable({ showPrices = false, currentKey, selectedKey, onSelect, highlightKeys }: {
  showPrices?: boolean;
  currentKey?: string;
  selectedKey?: string;
  ...
  const columnClass = (key: string) => {
    if (selectable) return key === selectedKey ? ' is-plan-selected' : key === currentKey ? ' is-plan-current' : '';
    return highlightKeys?.includes(key) ? ' is-plan-popular' : '';
  };
  return (
